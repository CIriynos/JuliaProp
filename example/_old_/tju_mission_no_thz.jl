...
########################
# retrieve data.
crt_shwave = retrieve_obj("saoyanchi_no_thz", "crt_shwave")
phi_record = retrieve_obj("saoyanchi_no_thz", "phi_record")
dphi_record = retrieve_obj("saoyanchi_no_thz", "dphi_record")
δa_lm = retrieve_obj("saoyanchi_no_thz", "delta_a_lm")

# a_tsurff_vec = isurf_sh_vector(pw, rt, phi_record, dphi_record, crt_shwave, At_data_x, At_data_y, At_data_z, Ri_tsurf, t_linspace, k_space, TSURF_MODE_ELLI);
# tsurf_plot_xy_momentum_spectrum_vector(a_tsurff_vec, k_space, kr_flag=true)
# δa_lm = isurf_rest_part(crt_shwave, k_space.k_r_range, last(t_linspace), Ri_tsurf, pw, rt)

# k_space_2 = create_k_space(0.002: 0.002: 1.50, fixed_theta(pi/2), phi_linspace(Nk_phi))
# δa_lm = isurf_rest_part(crt_shwave, k_space_2.k_r_range, last(t_linspace), Ri_tsurf, pw, rt)

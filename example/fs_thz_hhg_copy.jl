...
etp = plot([Ex_fs.(t_linspace) (Ex_c.(t_linspace)) .* 100])
push!(Ats, atp)
push!(Ets, etp)
...
# # tdse_elli_sh_mainloop_record_xy(crt_shwave, pw, rt, At_data_x, At_data_y, steps, Ri_tsurf);
...
# # Store Data
# example_name = "fuck_nothz_tau$(tau)"
...
plot([norm.(tps[1]) norm.(tps[2])])

# k0 = (first(pp) - last(pp)) / (length(pp) - 1)
# # y = first(pp) + k0 * (x - 1)
# plot(tau_list[1: length(tau_list) - 1], pp .- [(first(pp) .- k0 .* ((1:14) .- 1));])
# # plot(pp)
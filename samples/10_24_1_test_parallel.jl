...
Δt = 0.05
Z = 1.0
po_func_r = coulomb_potiential_zero_fixed_plus(Rco=50.0)
...
# pre-complie
tmmp = deepcopy(crt_shwave)
@time tdse_elli_sh_mainloop_record_xy(tmmp, pw, rt, At_data_x, At_data_y, 1, 450.0);

# propagation
Ri_tsurf = 450.0
@time phi_record, dphi_record = tdse_elli_sh_mainloop_record_xy(crt_shwave, pw, rt, At_data_x, At_data_y, 20, Ri_tsurf);

# @time a_tsurff_vec = isurf_sh_vector(pw, rt, phi_record, dphi_record, crt_shwave, At_data_x, At_data_y, At_data_z, Ri_tsurf, t_linspace, k_space, TSURF_MODE_ELLI);
...
# Data:
# core_nums = [1, 2, 4, 8, 16, 32, 64]
# l1 = [212.162564, 115.004274 ,66.937944 ,34.481858 ,23.477622 ,20.654866 ,42.814558 ]
# l2 = [30.394592 , 15.441667 , 8.208312 ,4.969207 ,3.328977 ,2.162080 ,2.182029 ]
# l3 = [23.241138 ,13.317621 ,7.286045 ,4.155531 ,2.377173 ,1.450260 ,1.128313 ]

# plot(log2.(core_nums), [l1[1] ./ l1 core_nums])
# plot(log2.(core_nums), [l2[1] ./ l2 core_nums])
# plot(log2.(core_nums), [l3[1] ./ l3 core_nums])

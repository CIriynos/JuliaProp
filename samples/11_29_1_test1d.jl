...
E0 = 0.057 * fs_rate
E0_thz = 0.0001 * thz_rate * 0
E0_c = 0.0001 * thz_rate * 1
nc = 15
Tp = 2 * nc * pi / omega
...
Plist_cc = []
crt_wave = deepcopy(init_wave)
wave_temporary = [zeros(ComplexF64, Nx) for i = 1: length(at_zeros_ids)]
wave_temporary[1] .= crt_wave
...
for i = 2: length(at_zeros_ids)
    ...
    wave_temporary[i] .= crt_wave 
    println("finishing TDSE i=$(i)")
end


for i = 1: length(at_zeros_ids)
    #wave_L = gauge_transform_V2L(wave_temporary[i], At_data[at_zeros_ids[i]], delta_t, x_linspace)
    Plist_total, Plist = windows_operator_method_1d(wave_temporary[i], gamma, 3, Ev_list, rt1d, pw1d)
    push!(Plist_total_cc, Plist_total)
    push!(Plist_cc, Plist)
    println("finishing WOM i=$(i)")
end

wave_L = gauge_transform_V2L(wave_temporary[31], At_data[1: at_zeros_ids[31]], delta_t, x_linspace)
Plist_total, Plist = windows_operator_method_1d(wave_L, gamma, 3, Ev_list, rt1d, pw1d)
plot(Plist, yscale=:log10, ylimits=(1e-10, 1))
plot(Plist_total, yscale=:log10, ylimits=(1e-10, 1))
id_zero = Int64((0.0 - E_min) ÷ E_delta)
sum(Plist[id_zero: length(Plist)])

id_zero = Int64((0.0 - E_min) ÷ E_delta)
range1 = length(Ev_list) + id_zero: length(Ev_list) * 2 - 1
range2 = 1: length(Ev_list) - id_zero
plot([Plist_total[range1] reverse(Plist_total[range2])], yscale=:log10, ylims=(1e-10, 1))

sum(Plist_total[range1])
sum(Plist_total[range2])

# Store Data
example_name = "11_29_1_test1d"
...
# Analyse
plot(Ev_list, Plist_cc[1], yscale=:log10, ylims=(1e-10, 1))
...
for i = 2: length(ionz_rate_list) - 5
...
range1 = length(Ev_list) + id_zero: length(Ev_list) * 2 - 1
range2 = 1: length(Ev_list) - id_zero
plot([Plist_total_cc[20][range1] reverse(Plist_total_cc[20][range2])], yscale=:log10, ylims=(1e-10, 1))

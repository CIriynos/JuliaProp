...
rmax = Nr * Δr  # 1000
Ri_tsurf = 800.0
po_func_r(r) = -1.0 * (r^2 + 5e-5) ^ (-0.5) * flap_top_windows_f(r, 0, Ri_tsurf, 1/4, left_flag=false)
...
steps_laser = steps_1
steps = steps_laser + Int64(tau_fs ÷ Δt)
...
# plot([real.(hhg_integral_t_3) imag.(hhg_integral_t_3)])
...
# hhg_windows_data = hhg_windows_f.(t_linspace, tau_fs - 0, Tp + tau_fs)

...
scalar_list = [1.0, 0.1, 0.01]
# Define the Laser. 
E_fs = 0.0533 * scalar_list[task_id]
E_thz = 0.00005 * 0
E_dc = 0.00005 * 0
ω_fs = 0.05693
ω_thz = ω_fs / 25
...
Ex_fs, Ey_fs, Ez_fs, tmax = light_pulse(ω_fs, E_fs, nc, tau_fs, ellipticity=1.0, phase1=0.5pi, phase2=0.0)
...
At_datas, Et_datas, ts, steps = create_tdata(tmax, 0, Δt, t -> Ex_fs(t), Ey_fs, no_light, appendix_steps=1)
...
# Retrieve Data.
example_name = "2025_10_9_3_$(task_id)"
hhg_integral_t = retrieve_mat(example_name, "hhg_integral_t_1")

# HHG
p, hhg_data_x, hhg_data_y, base_id, hhg_k_linspace = get_hhg_spectrum_xy(hhg_integral_t, Et_datas[1], Et_datas[2], tau_fs, tmax, ω_fs, ts, Δt, max_display_rate=15, min_log_limit=1e-12)
...
pp = plot(hhg_k_linspace .* 0.05693, norm.(hhg_data_x_list[2][1:length(hhg_k_linspace)]), yscale=:log10, ylimits=(1e-15, 1e3))

R = 0.5
plot!(pp, [R, R], [1e-15, 1e3], label="m=1, n=∞")
R1 = R * (1/1^2 - 1/2^2)
R2 = R * (1/1^2 - 1/3^2)
R3 = R * (1/1^2 - 1/4^2)
R4 = R * (1/1^2 - 1/5^2)
plot!(pp, [R1, R1], [1e-15, 1e3], label="m=1, n=2")
plot!(pp, [R2, R2], [1e-15, 1e3], label="m=1, n=3")
plot!(pp, [R3, R3], [1e-15, 1e3], label="m=1, n=4")
plot!(pp, [R4, R4], [1e-15, 1e3], label="m=1, n=5")


# plot(800 * (1 ./ hhg_k_linspace[1:301]), norm.(hhg_data_x_list[1])[1:length(hhg_k_linspace[1:301])],
#     yscale=:log10, label="E0", xscale=:log10, ylimits=(1e-12, 1e1))

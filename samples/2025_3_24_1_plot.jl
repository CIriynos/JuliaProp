...
Ex_fs, Ey_fs, Ez_fs, tmax = light_pulse(ω_fs, E_fs, nc, tau_fs, ellipticity=0.5, phase1=0.5pi, phase2=0.0)
...
ppp = plot(norm.(Et_datas[1].^2 .+ Et_datas[2].^2))
...
# Retrieve Data.
example_name = "2025_10_24_1_$(task_id)"
...
p = heatmap(ts__, freqs[5:cd] ./ f0, log10.(norm.(res')[5:cd, :]), yscale=:log10)
...
plot!(p1, x2, 2.9 * ones(length(x2)), color=:white, lw=0.8, linestyle=:dash,
    legend=false, xlims = (ts__[1], ts__[end]), ylims = (yy2[1], yy2[end]))
...
# plot!(p2, x2, [d1 d1 .+ (d2 .- d1) * 5 (exp.(z2[33, :])) .* mask],
#     xlims = (ts__[1], ts__[end]), ylims=[0.00015, 0.00035], color=[:red :orange1 :green1],
#     yticks=[0.0002, 0.0003], ylabel="|X(a,b)|", xlabel="b (a.u.)",
#     label=["a=2 (τ1)" "a=2 (τ2)" "a=3"], legend=:topright)
...
# l = @layout [a{.5h}; b{.2h}; c{.3h}]
# plot(p1, p2, p0, layout=l, link=:x, legendfontsize=8, labelfontsize=10, framestyle=[:box :box :box :box])
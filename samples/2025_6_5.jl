...
Ex_fs, Ey_fs, Ez_fs = light_pulse(ω_fs, E_fs, nc, tau_fs, ellipticity=0.2, phase1=0.5pi + cep, phase2=0 + cep)
...
# Special PDD Analyse
if m == 1
    pdd_tmp = trajs_pdd_analyse(rt, tid_cc, traj_filter_flag, tau_fs, nc, ω_fs, Δt)
else
    pdd_tmp .+= trajs_pdd_analyse(rt, tid_cc, traj_filter_flag, tau_fs, nc, ω_fs, Δt)
end
end
...
cl = cgrad([:white, :blue, :yellow], [0.0, 0.5, 0.9, 0.95, 1.0])
heatmap(transpose(pdd_list[1][tstart: rt.t_num, :]) .^ 1.0, size=(800, 200), color=cl, clim=(0, 5.5e5))

plot(pdd_list[1][rt.t_num - 2000, :])
...
bound_state_num = sum([i^2 for i = 1: 2])
example_name = "2026_3_30_test_itp_short_range"
eigen_states = retrieve_compact_bound_states(example_name, bound_state_num, pw.shgrid.l_num)
...
# plot(ks ./ ω_fs, [(ks .^ 4) .* hg1, (ks .^ 4) .* hg1_free, (ks .^ 4) .* hg1_bound], yscale=:log10, yaxis=[1e-4, 1e-2, 1e0, 1e2, 1e4], ylimit=(1e-8, 1e3))

plot(ks ./ ω_fs, [hg1, hg1_free, hg1_bound], yscale=:log10, yaxis=[1e-4, 1e-2, 1e0, 1e2, 1e4], ylimit=(1e-8, 1e3))

...
Ex_fs, Ey_fs, Ez_fs, tmax = light_pulse(ω_fs, E_fs, nc, tau_fs, ellipticity=0.85, phase1=0.0)        # create the light pulse from the given parameters (+ ellipticity)
...
hhg_integral_t_1, hhg_integral_t_2, hhg_integral_t_3,
...
_, _ = tdse_elli_sh_mainloop_hhg_analysis(crt_shwave, pw, rt, At_datas[1], At_datas[2], steps, Ri_tsurf, eigen_states)
...
h5open("./data/$example_name.h5", "w") do file
    ...
    write(file, "hhg_integral_t_1_free", hhg_integral_t_1_free)
    write(file, "hhg_integral_t_2_free", hhg_integral_t_2_free)
    write(file, "hhg_integral_t_3_free", hhg_integral_t_3_free)
    write(file, "hhg_integral_t_1_bound", hhg_integral_t_1_bound)
    write(file, "hhg_integral_t_2_bound", hhg_integral_t_2_bound)
    write(file, "hhg_integral_t_3_bound", hhg_integral_t_3_bound)
end

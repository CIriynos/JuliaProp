...
@expo ω_fs =          0.057 * 0.5               # angular frequency of the fs pulse
...
for (i, p) in enumerate(p_grid)
    ...
    for α in alpha_list
        ...
        for (j, r) in enumerate(rs)
            RL_left[α][i] += r ^ 2 * eigen_states[α].data[j] * spherical_besselj_table[(l + 1) + 1][j] * sqrt(pw_.shgrid.rgrid.delta) #* r_mask[α][j]
        end
        if l != 0
            for (j, r) in enumerate(rs)
                RL_right[α][i] += r ^ 2 * eigen_states[α].data[j] * spherical_besselj_table[(l - 1) + 1][j] * sqrt(pw_.shgrid.rgrid.delta) #* r_mask[α][j]
            end
        end

        for (j, r) in enumerate(rs)
            RL_fix[α][i] += r * eigen_states[α].data[j] * spherical_besselj_table[l + 1][j] * sqrt(pw_.shgrid.rgrid.delta) #* r_mask[α][j]
        end
    end
end
...
h5open("./data/2025_7_14_denser_8times.h5", "w") do h
...
data_name = "2025_7_14_denser_8times"

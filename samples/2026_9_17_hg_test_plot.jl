...
E_fs = 0.0533 * 1.0
E_thz = 0.00005 * 1 * 10
E_dc =  0.00002 * E_dc_sign * 10
ω_fs = 0.05693
ω_thz = ω_fs / 30
nc = 20
tau_fs = 0
tau_list = get_1c_thz_delay_list_ok(ω_fs, tau_fs, nc, ω_thz, samples_num=samples_num)
tau_thz = tau_list[task_id]

Ex_fs, Ey_fs, Ez_fs, tmax = light_pulse(ω_fs, E_fs, nc, tau_fs, ellipticity=0.3, phase1=0.5pi, phase2=pi)
...
# Retrieve Data.
example_name = "2026_9_17_hg_re_$(task_id)_E_dc_sign_$(E_dc_sign)"
...
plot(hhg_k_linspace, norm.(hhg_data_x_list[1][1:length(hhg_k_linspace)]), yscale=:log10)
...
plot([d1, d2, d1 .- d2], labels=["E_dc +" "E_dc -" "diff"])

# unify(data) = (data .- minimum(data)) ./ (maximum(data) - minimum(data))
# p2 = plot(unify(tau_list), unify(d1 .- d2))

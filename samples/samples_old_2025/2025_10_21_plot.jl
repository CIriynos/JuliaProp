...
# Retrieve Data.
example_name = "2025_10_21_$(task_id)"
hhg_integral_t = retrieve_mat(example_name, "hhg_integral_t")

# HHG
windows_time_delta = 0
p, hhg_data_x, hhg_data_y, base_id, hhg_k_linspace = get_hhg_spectrum_xy(hhg_integral_t, Et_datas[1], Et_datas[2],
    tau_fs + windows_time_delta, tmax - windows_time_delta, ω_fs, ts, Δt, max_display_rate=20, min_log_limit=1e-12)
...
# p3 = scatter(log2.(1 ./ (2:16)), log2.(shg_yields)[2:16])
# plot!(p3, log2.(0.02: 0.001: 0.5), log2.(0.00022 .* (0.02: 0.001: 0.5) .^ 2), label="y=0.0005*x^2", lw=2, ls=:dash, color=:red)

min_display_value = 1e-8
pp = plot(hhg_k_linspace .* 0.05693, norm.(hhg_data_x_list[1][1:length(hhg_k_linspace)]), yscale=:log10, ylimits=(min_display_value, 1e3))

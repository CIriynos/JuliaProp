...
# hhg_windows_data = hhg_windows_f.(t_linspace, 0 + tau_fs, T_total - tau_fs - appendix_time)
hhg_windows_data = hhg_windows_f.(t_linspace, 0, T_total)
...
# plot(hhg_k_linspace[1:max_hhg_id] ./ ω1, norm.(hhg_spectrum)[1: max_hhg_id], yscale=:log10, ylimits=(1e-5, 1e2))

pp = plot(hhg_k_linspace[1:max_hhg_id], (norm.(hhg_spectrum[1:max_hhg_id])), yscale=:log10, ylimits=(1e-10, 1e2))
plot!(pp, [abs(en0), abs(en0)], [1e-15, 1e3], label="1")
...
# 10_26_1.jl
# 测试ω_thz，或者fs包络、E_thz什么时候才能测到双峰（测到THz波形）
...
push!(Ats, atp)
push!(Ets, etp)
push!(Ex_fs_prob, Et_data_x)
push!(Ey_fs_prob, Et_data_y)
push!(E_thz_prob, Ex_thz.(t_linspace))
push!(Ex_prob, Et_data_x)
push!(Ey_prob, Et_data_y)
...
# # Store Data
# example_name = "10_26_1_$(tau)_$(E_ratio)"
...
# # iii = 3
# # ionz1 = Wk2.(abs.(Ex_prob[iii]) .+ 1e-10, 0.5, 0.05) 
# # ionz2 = ADK_f.(abs.(Ex_prob[iii]) .+ 1e-10)
# # rate = maximum(ionz1) / maximum(ionz2)
# # Eorigin = Ex_prob[iii] ./ 1e4
# # plot([ionz2])

# F = 0.01: 0.001: 0.1
# rate = Wk2.(0.05, 0.5, 0.05) / ADK_f.(0.05)
# plot(0.05 ./ F, [Wk2.(F, 0.5, 0.05) ADK_f.(F) * rate],
#     yscale=:log10,
#     xlabel="gamma",
#     ylabel="ionz. abs. Rate",
#     labels=["Keldysh Formula" "ADK Formula"])